...
- Docker Desktop (для Windows/Mac) или Docker Engine (для Linux) с поддержкой Docker Compose
- Node.js (v18+)
- pnpm (рекомендуется)

> **ВАЖНО:** Убедитесь, что Docker daemon запущен перед выполнением команд. Для пользователей Windows: убедитесь, что Docker Desktop установлен и запущен, иначе вы получите ошибку вида "The system cannot find the file specified" при попытке запустить docker-compose.
...
> **Примечание:** В корне проекта находится файл `.env` с базовыми настройками. Если он отсутствует, используйте команду выше для его создания.
